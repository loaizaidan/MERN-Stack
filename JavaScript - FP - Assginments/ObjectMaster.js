...
const poisonTypeNames = pokémon.filter(p => p.types.length === 1 && p.types[0] === "poison").map(p => p.name);
const firstTypeIfSecondIsFlying = pokémon.filter(p => p.types[1] === "flying").map(p => p.types[0]);
